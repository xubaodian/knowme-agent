...
export function ModelPicker({ className, models, onChange, selectedModel }: ModelPickerProps) {
  const [isOpen, setIsOpen] = useState(false);
  const rootRef = useRef<HTMLDivElement | null>(null);
  const selectedOption = useMemo(() => models.find((model) => model.id === selectedModel), [models, selectedModel]);
  const isDisabled = models.length === 0;
  const label = selectedOption?.label ?? selectedModel ?? (isDisabled ? "No models" : "Choose model");
            ...
            return (
              ...
              >
                ...
                <span className="min-w-0">
                  <span className="block truncate text-xs font-semibold text-foreground">{model.label}</span>
